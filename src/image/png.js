...
class PNGImage {
  ...
  embed(document) {
    ...
    if (this.image.transparency.grayscale) {
      ...
      let val = this.image.transparency.greyscale;
      return this.obj.data['Mask'] = [val, val];

    } else if (this.image.transparency.rgb) {
      // Use Color Key Masking (spec section 4.8.5)
      // An array with N elements, where N is two times the number of color components.
      let { rgb } = this.image.transparency;
      let mask = [];
  ...
  splitAlphaChannel() {
    return this.image.decodePixels(pixels => {
      let a, p;
      let colorByteSize = (this.image.colors * this.image.bits) / 8;
      let pixelCount = this.width * this.height;
      let imgData = new Buffer(pixelCount * colorByteSize);
      let alphaChannel = new Buffer(pixelCount);
  ...
  loadIndexedAlphaChannel(fn) {
    let transparency = this.image.transparency.indexed;
    return this.image.decodePixels(pixels => {
      let alphaChannel = new Buffer(this.width * this.height);
