...
export default {
  ...
  _setColor(color, stroke) {
    ...
    let op = stroke ? 'SCN' : 'scn';

    if (color instanceof PDFGradient) {
      this._setColorSpace('Pattern', stroke);
      color.apply(op);
    } else {
      let space = color.length === 4 ? 'DeviceCMYK' : 'DeviceRGB';
  ...
  fillColor(color, opacity) {
    ...
    let set = this._setColor(color, false);
  ...
  strokeColor(color, opacity) {
    if (opacity == null) { opacity = 1; }
    let set = this._setColor(color, true);
  ...
  _doOpacity(fillOpacity, strokeOpacity) {
     ...
     let key = `${fillOpacity}_${strokeOpacity}`;
